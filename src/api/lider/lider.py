...
class LiderInstaller(object):
    ...
    def __init__(self, ssh_api, ssh_status):
        ...
        self.lider_war_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../conf/ROOT.war')
        self.tomcat_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../conf/apache-tomcat-9.0.36.tar.gz')
    ...
    def install(self, data):
        ...
        if self.ssh_status == "Successfully Authenticated":
            ...
            result_code = self.ssh_api.run_command("sudo tar xf /tmp/apache-tomcat-*tar.gz -C /opt/tomcat --strip-components=1")
            ...
            result_code = self.ssh_api.run_command("sudo cp /tmp/ROOT.war /opt/tomcat/webapps/")
